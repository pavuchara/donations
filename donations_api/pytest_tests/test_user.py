...
def test_user_creation_correct_data(anonymous_client, user_register_path):
...
def test_user_creation_wrong_data(anonymous_client, user_register_path, user_data):
...
def test_user_creation_with_existing_data(anonymous_client, user_register_path):
    response = anonymous_client.post(user_register_path, CORRECT_USER_CREATE_DATA)
    assert response.status_code == status.HTTP_400_BAD_REQUEST, (
...
def test_update_user_profile_owner_correct_data(auth_user_client, user_update_retrive_path):
...
def test_update_user_profile_owner_wrong_data(
    auth_user_client, user_update_retrive_path, user_data, user_db
):
...
def test_patrial_update_user_profile_owner_correct_data(
    auth_user_client, user_update_retrive_path, user_db
):
...
def test_patrial_update_user_profile_owner_wrong_data(
    auth_user_client, user_update_retrive_path, user_db, user_data, data_key
):
...
def test_user_cant_edit_another_profile(auth_user_second_client, user_update_retrive_path, user_db):
...
def test_user_cant_partial_edit_another_profile(auth_user_second_client, user_update_retrive_path, user_db):
...
def test_anonymous_user_cant_update_profile(anonymous_client, user_update_retrive_path, user_db):
...
def test_anonymous_user_cant_partial_update_profile(anonymous_client, user_update_retrive_path, user_db):
    for k, v in CORRECT_USER_PARTIAL_UPDARE_DATA.items():
...
@pytest.mark.usefixtures('user_db')
def test_user_can_obtain_refresh_token(anonymous_client, token_obtain_path, token_refresh_path):
